...
class InfoGen(nn.Module):
    def __init__(
        ...
        # self.tconv2 = nn.ConvTranspose2d(512, 256, 3, 2, padding=1, bias=False)
        # self.bn2 = nn.BatchNorm2d(256)
    

        # self.tconv3 = nn.ConvTranspose2d(256, 128, 3, 2, padding=1, bias=False)
        # self.bn3 = nn.BatchNorm2d(128)

        # self.tconv4 = nn.ConvTranspose2d(128, output_size, 3, (2, 1), padding=(1, 0), bias=False)
        # self.bn4 = nn.BatchNorm2d(output_size)


        # h 1 w 33 to h 16 w 64; 26 to 50
        self.tconv1 = nn.ConvTranspose2d(t_emb, 512, kernel_size=(3,4), stride=(1,1), padding=(1,2),bias=False)
        self.bn1 = nn.BatchNorm2d(512)
        ...
        self.tconv3 = nn.ConvTranspose2d(256, 128, kernel_size=(4,3), stride=(2,1), padding=(1,1), bias=False)
        # self.tconv3 = nn.ConvTranspose2d(256, 128, kernel_size=(4,4), stride=(2,2), padding=(1,1), bias=False)
        self.bn3 = nn.BatchNorm2d(128)

        self.tconv4 = nn.ConvTranspose2d(128, 64, (4,3), stride=(2,1), padding=(1,1), bias=False)
        self.bn4 = nn.BatchNorm2d(64)

        self.tconv5 = nn.ConvTranspose2d(64, 32, (4,3), stride=(2,1), padding=(1,1), bias=False)
    ...
    def forward(self, t_embedding):
        ...
        x = F.relu(self.bn1(self.tconv1(t_embedding))) # 1 63 1 33 to 1 32 16 64 ;  1 63 1 33 to 1 32 16 128
        # print(x.shape)
...
class TSRN_TL(nn.Module):
    ...
    def forward(self, x, text_emb=None):
        ...
        if text_emb is None:
            N, C, H, W = x.shape  # B 4 16 64
            w_feat = int(W/2+1)
            text_emb = torch.zeros((N, self.emb_cls, 1, w_feat)) # 1 63 1 33

        spatial_t_emb = self.infoGen(text_emb) # 1 33  to 16 64; text_emb = 48 63 1 12 to spatial_t_emb= 48 32 16 22  ; 初始化时 1 63 1 33 to 1 32 16 64
        spatial_t_emb = F.interpolate(spatial_t_emb, (x.shape[2], x.shape[3]), mode='bilinear', align_corners=True) # 1 32 16 64 to 1 32 16 64
...
class RecurrentResidualBlockTL(nn.Module):
    def __init__(self, channels, text_channels):
        ...
        self.dilation1 = nn.Sequential(SeparableConv2d(self.width, self.width, kernel_size=3, padding=1, dilation=1, bias=False),
    ...
    def forward(self, x, text_emb):
        ...
        cat_feature = self.dilation1(cat_feature) 
...
class GruBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.gru = nn.GRU(out_channels, out_channels // 2, bidirectional=True, batch_first=True)  # nn.GRU(out_channels, out_channels // 2, bidirectional=True, batch_first=True)

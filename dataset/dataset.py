...
class alignCollate_realWTLAMaskv2(alignCollate_syn): # go here 
...
class alignCollate_realWTLAMask(alignCollate_syn): # go here 
...
# 文件读取类
class ImageDataSet80k(Dataset):
    def __init__(self, info_file, transform=None): # info_file 输入annotation_train2.txt路径
        self.transform = transform
        self.img_names = []
        self.img_labels = []
        self.labels_length = []        

        with open(info_file, 'r', encoding='utf-8') as fp:  # ./2425/1/115_Lube_45484.jpg 45484
            lines = fp.readlines()
            for line in lines:
                img_name = line.split(' ')[0]
                # img_name =self.img_fold+img_name[1:]
            ...
            # for line in lines:
            #     img_name = line.split(' ')[0]  # ./2425/1/115_Lube_45484.jpg
            #     # img_name =self.img_fold+img_name[1:]
    ...
    def __getitem__(self, index):
        path = '/home/tcs1/data2/SR_datasets/90k/90kDICT32px'
        imgW = 128
        imgH = 32
        image_path = path + self.img_names[index][1:]
        ...
        random_times = random.randint(0, 5)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  
        img_lr, _ = blindsr.degradation_bsrgan(img, 2, lq_patchsize=32,isp_model=None,more_blur = random_times )
...
# 重构的文件结构组织函数 输出batch图像和标签
def collate_fn_80k(data_list):
    ...
    image_label = []
    image_width = []
    # imgs_lr = [np.zeros((1, 1)) for _ in range(5)] 
    
    for i in range(length): # batch size
        ...
        img_data[i] = cv2.resize(img_data[i], (imgW, imgH), interpolation=cv2.INTER_CUBIC)

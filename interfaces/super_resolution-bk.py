                ...
                if not self.args.go_test:
                    ...
                    if self.args.syn:
                        ...
                        if self.args.arch == "tsrn":
                            ...
                            # bs = images_hr.shape[0]
                            # for i in range(bs):
                            #     images_hr_single = images_hr[i:i+1,:,:,:]
                            #     images_hr_single=torch.squeeze(images_hr_single,dim=0)
                            #     images_hr_array=images_hr_single.permute(1,2,0).detach().cpu().numpy() # from  c h w to   h w c 
                            #     images_hr_array_c3 = images_hr_array[:,:,:3]
                            #     # images_lr_array_c4 = images_hr_array[:,:,3:4]
                            #     images_lr_array, _ = blindsr.degradation_bsrgan(images_hr_array_c3, 2, lq_patchsize=32,isp_model=None)
                            #     # images_lr_array = np.concatenate(images_lr_array,images_hr_array_c4,axis=2) 
                            #     images_lr_single = torch.from_numpy(images_lr_array).permute(2,0,1)
                            #     images_lr[i:i+1,:3,:,:]=torch.unsqueeze(images_lr_single,dim=0)


                            # output_array = images_hr.permute(0,2,3,1).detach().cpu().numpy() # from b c h w to  b h w c
                            # output_array2 = output_array[:,:,:,:3] # b h w c
                            # images_lr_array2, _ = blindsr.degradation_bsrgan(output_array2, 2, lq_patchsize=32,isp_model=None)
                            # images_lr2 = torch.from_numpy(images_lr_array2).permute(0,3,1,2)
                            # images_lr4 =images_lr[:,-1,:,:]                            
                            # images_lr = torch.cat((images_lr2,images_lr4),dim=1)

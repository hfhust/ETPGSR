...
from model.crnn import DenseNet_BLSTM_CTC_MODEL
...
class TextBase(object):
    def __init__(self, config, args, opt_TPG=None):
        ...
        if self.args.syn:
            ...
            elif self.args.arch == 'tsrn_tl_cascade': # dataset is set here
                # self.align_collate = collate_fn_80k #alignCollate_realWTLAMask
                # self.load_dataset = ImageDataSet80k #lmdbDataset_real
                self.align_collate = alignCollate_realWTLAMask
                self.load_dataset = lmdbDataset_real
    ...
    def get_train_data(self):
        ...
        # train_dataset = ImageDataSet80k(info_file='/home/tcs1/data2/SR_datasets/90k/90kDICT32px/annotation_val-new3.txt')
    ...
    def generator_init(self, iter=-1):
        ...
        if self.args.arch != 'bicubic':
            ...
            if self.resume is not '':
                print('loading pre-trained SR model from %s ' % self.resume)
    ...
    def save_checkpoint(self, netG_list, epoch, iters, best_acc_dict, best_model_info, is_best, converge_list, recognizer=None):
        ...
        for i in range(len(netG_list)):
            netG = netG_list[i]
            save_dict = {
                'state_dict_G': netG.state_dict(),  # netG.module.state_dict(),
                ...
                'param_num': sum([param.nelement() for param in netG.parameters()]), # netG.module.parameters()
    ...
    def CRNN_DenseNet_init(self, recognizer_path=None, opt=None):
        # model = crnn.CRNN(32, 1, 37, 256)
        model = DenseNet_BLSTM_CTC_MODEL(growth_rate=32,  # 32
        ...
        model_path = recognizer_path if not recognizer_path is None else self.config.TRAIN.VAL.crnn_densenet_pretrained

        print('loading pretrained  crnn_DenseNet TPG model from %s' % model_path)

        if model_path.split('.')[-1] == 'tar':
        ...
        print('crnn_DenseNet TPG model has restored')       
        ...
        #     model.load_state_dict(stat_dict)      
        # except Exception:
        #     raise ValueError("模型状态字典与模型结构不匹配，无法加载")      
    ...
    def parse_crnn_data(self, imgs_input):
        ...
        in_width = self.config.TRAIN.width if self.config.TRAIN.width != 128 else 100 # 此处图像width被修改为100 CRNN识别网络   else 100

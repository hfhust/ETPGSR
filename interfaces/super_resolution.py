...
from model.crnn.densenet_crnn import DenseNet_BLSTM_CTC_MODEL
...
class TextSR(base.TextBase):
    ...
    def train(self):
        ...
        TP_Generator_dict = {
            "CRNN": self.CRNN_DenseNet_init,  #CRNN_init  CRNN_DenseNet_init
            #"OPT": self.TPG_init
        }
        ...
        train_dataset, train_loader = self.get_train_data()  # [image_data, image_label, label_length]
        val_dataset_list, val_loader_list = self.get_val_data()
        model_dict = self.generator_init(0) # 初始化SR模型，并计算LR输入为16 64时的FLOPs，其中TPG 使用zeros输入
        ...
        if self.args.arch in ["tsrn_tl_wmask", "tsrn_tl"]:
            ...
            for i in range(stu_iter):
                ...
                if os.path.isfile(recognizer_path):
                    aster_student_, aster_stu_info = TP_Generator_dict[self.args.tpg](recognizer_path=recognizer_path, opt=tpg_opt) # 加载训练好的recognizer作为TPG
                else:
            ...
            for j, data in (enumerate(train_loader)):
                ...
                if not self.args.go_test:
                    ...
                    if self.args.syn:
                        ...
                        elif self.args.arch in ABLATION_SET:  # [image_data, image_label, label_length]
                            images_hr, images_lr, label_strs, label_vecs, weighted_mask, weighted_tics = data 
                            # images_hr,  label_vecs, label_length = data                           
                    ...
                    if self.args.arch == "tsrn":
                        ...
                        aster_dict_hr = self.parse_crnn_data(images_hr[:, :3, :, :]) # 返回灰度图
        ...
        for i, data in (enumerate(val_loader)):
            ...
            if self.args.syn:
                ...
                if self.args.arch == "tsrn":
                    ...
                    images_hr, images_lr, label_strs, label_vecs_gt = data  # val data load here
                else:
                    images_hr, images_lr, label_strs = data
            ...
            if self.args.arch == "tsrn":
                ...
                if self.args.random_reso:
                    ...
                    with torch.no_grad():
                        ...
                        label_vecs_hr_pos = torch.nn.functional.softmax(label_vecs_hr, -1)[..., 1:] # 不包括- 字符
                    
                        char_prob = label_vecs_hr_pos.permute(1, 0, 2).unsqueeze(1).permute(0, 3, 1, 2)  # 1 62 1 12
                        prob_val, prob_ind = char_prob.max(1) 
                        # prob_val = (prob_val >= 0.1).float()
                        prob_val = torch.nn.functional.interpolate(prob_val.unsqueeze(1), images_hr.shape[-2:], mode='bilinear')  # hr 32 128 lr  16 64
                ...
                if self.args.arch in ABLATION_SET:
                    ...
                    for i in range(self.args.stu_iter):
                        ...
                        if self.args.test_model == "CRNN":
                            ...
                            preds = aster_output_sr[0]  # , preds_reverse by Feng He 
                            _, preds = preds.max(1)
            ...
            if self.args.test_model == "CRNN":
                ...
                ### LR ###
                preds = aster_output_lr[0] # , preds_reverse by Feng He 
                _, preds = preds.max(1)
                ...
                ### HR ###
                preds = aster_output_hr[0] # , preds_reverse  by Feng He 
                _, preds = preds.max(1)
            ...
            for batch_i in range(len(images_lr)):
                ...
                if vis:
                    ...
                    # if SR_stat[batch_i] == "SR":   # 注释掉，报错 Feng He 0731
